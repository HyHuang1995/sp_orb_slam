...
MapPoint::MapPoint(const cv::Mat &Pos, KeyFrame *pRefKF)
    : mnFirstKFid(pRefKF->mnId), mnFirstFrame(pRefKF->mnFrameId), nObs(0),
      mnTrackReferenceForFrame(0), mnTrackReferenceDust(0), mnLastFrameSeen(0),
  ...
  unique_lock<mutex> lock(global::map->mMutexPointCreation);
  mnId = nNextId++;
}
...
void MapPoint::updateDescTrack(KeyFrame *pKF, const int idx) {
  unique_lock<mutex> lock2(mutex_desc_track);
  mDescTrack = pKF->mDescriptors.row(idx).clone();
}

cv::Mat MapPoint::getDescTrack() {
  unique_lock<mutex> lock2(mutex_desc_track);
  return mDescTrack.clone();
}

void MapPoint::SetWorldPos(const cv::Mat &Pos) {
  unique_lock<mutex> lock2(mGlobalMutex);
  unique_lock<shared_mutex> lock(mMutexPos);
  Pos.copyTo(mWorldPos);
}

cv::Mat MapPoint::GetWorldPos() {
  shared_lock<shared_mutex> lock(mMutexPos);
...
void MapPoint::SetBadFlag() {
  map<KeyFrame *, size_t> obs;
  {
    ...
    shared_lock<shared_mutex> lock2(mMutexPos);
    mbBad = true;
    obs = mObservations;
    mObservations.clear();
  }
...
void MapPoint::ComputeDistinctiveDescriptors() {
  ...
  Distances.resize(N, vector<float>(N, 0.0f));
  ...
  float BestMedian = numeric_limits<float>::max();
  int BestIdx = 0;
  for (size_t i = 0; i < N; i++) {
...
void MapPoint::UpdateNormalAndDepth() {
  ...
  {
    unique_lock<shared_mutex> lock3(mMutexPos);
    mfMaxDistance = dist * levelScaleFactor;
