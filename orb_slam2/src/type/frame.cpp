...
Frame::Frame(const Frame &frame)
    : global_desc(frame.global_desc.clone()),
      flann(static_cast<cv::FlannBasedMatcher *>(nullptr)),
      ...
      cov2_inv_(frame.cov2_inv_), dust_(frame.dust_.clone()),
      heat_(frame.heat_.clone()), occ_grid(frame.occ_grid.clone()),
      mvuRight(frame.mvuRight), mvDepth(frame.mvDepth),
      mDescriptors(frame.mDescriptors.clone()),
      mDescriptorsRight(frame.mDescriptorsRight.clone()),
      mvpMapPoints(frame.mvpMapPoints), is_mp_visible_(frame.is_mp_visible_),
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
             const double &timeStamp, BaseExtractor *extractorLeft,
             BaseExtractor *extractorRight, cv::Mat &K, cv::Mat &distCoef,
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth,
             const double &timeStamp, BaseExtractor *extractor, cv::Mat &K,
             cv::Mat &distCoef, const float &bf, const float &thDepth)
    : flann(static_cast<cv::FlannBasedMatcher *>(nullptr)),
      mpORBextractorLeft(extractor),
      mpORBextractorRight(static_cast<BaseExtractor *>(NULL)),
      mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()),
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp,
  ...
  // ADD(Depth filter)
  seeds_ = vector<std::shared_ptr<Seed>>(N, nullptr);
...
void Frame::ExtractORB(int flag, const cv::Mat &im) {
  if (flag == 0) {
    ...
    cov2_inv_ = dynamic_cast<SPExtractor *>(mpORBextractorLeft)->getCov2Inv();

    dust_ =
        dynamic_cast<SPExtractor *>(mpORBextractorLeft)->dense_dust_.clone();
    heat_ = dynamic_cast<SPExtractor *>(mpORBextractorLeft)->heat_.clone();

    // occ_grid =
    //     dynamic_cast<SPExtractor *>(mpORBextractorLeft)->occ_grid_.clone();

    dynamic_cast<SPExtractor *>(mpORBextractorLeft)->occ_grid_.copyTo(occ_grid);
