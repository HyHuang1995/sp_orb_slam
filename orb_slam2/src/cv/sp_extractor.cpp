...
void nms(const cv::Mat &det, const cv::Mat &desc, const int num_features,
  ...
  for (int v = 0; v < (img_height + dist_thresh); v++) {
    for (int u = 0; u < (img_width + dist_thresh); u++) {
      ...
      if (grid.at<char>(v, u) == 2) {
        ...
        pts.push_back(
            cv::KeyPoint(pts_raw[select_ind].x, pts_raw[select_ind].y, 1.0f));
...
SPExtractor::SPExtractor(int _nfeatures)
    : BaseExtractor(_nfeatures, 1.0, 1.0, 1.0, 1.0), num_feature_(_nfeatures),
      device_(torch::kCPU) {
  // SPExtractor::SPExtractor(int nfeatures, std::string weight_path,
  //                          float conf_thresh = 0.007) {
  // if (sp::device_type == CUDA) 
  {
...
void SPExtractor::operator()(InputArray _image, InputArray _mask,
  ...
  timing::Timer timer_extration("extraction");
  ...
  //                   height)!!!!
  cv::Mat heat_mat(cv::Size(width, height), CV_32FC1,
                   heat_cpu.data_ptr<float>());
  ...
  cv::Mat desc_;
  nms(pts_sorted, desc_sorted, num_feature_, kps, desc_, 8, 4, im_raw.cols,
      im_raw.rows, occ_grid_);
