...
int Optimizer::PoseOptimization(Frame *pFrame) {
  ...
  for (size_t it = 0; it < 4; it++) {
    ...
    int n_iter = optimizer.optimize(its[it]);
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag,
  ...
  vector<KeyFrame *> lFixedCamerasViz;
    ...
    for (map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      if (pKFi->mnBALocalForKF != pKF->mnId &&
        ...
        if (!pKFi->isBad()) {
          ...
          lFixedCamerasViz.push_back(pKFi);
        }
      }
    }
  }
  pMap->setBAKFs(vNeighKFs, lFixedCamerasViz);
    ...
    for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(),
      ...
      if (!pKFi->isBad()) {
        ...
        if (pKFi->mvuRight[mit->second] < 0) {
          ...
          {
            const auto inv_sigma2 = pKFi->cov2_inv_[mit->second];
            Eigen::Matrix2d info = Eigen::Matrix2d::Identity();
            info(0, 0) *= inv_sigma2.x();
            info(1, 1) *= inv_sigma2.y();
            e->setInformation(info);
...
void Optimizer::OptimizeEssentialGraph(
  ...
  //   g2o::BlockSolver_7_3 *solver_ptr = new
  //   g2o::BlockSolver_7_3(linearSolver);
  g2o::OptimizationAlgorithmLevenberg *solver;
  //   new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
  solver = new g2o::OptimizationAlgorithmLevenberg(
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    LoopClosingVLAD::KeyFrameAndPose::const_iterator it =
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    LoopClosingVLAD::KeyFrameAndPose::const_iterator iti =
    ...
    if (pParentKF) {
      ...
      LoopClosingVLAD::KeyFrameAndPose::const_iterator itj =
    ...
    for (set<KeyFrame *>::const_iterator sit = sLoopEdges.begin(),
      ...
      if (pLKF->mnId < pKF->mnId) {
        ...
        LoopClosingVLAD::KeyFrameAndPose::const_iterator itl =
    ...
    for (vector<KeyFrame *>::const_iterator vit = vpConnectedKFs.begin();
      ...
      if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) &&
        ...
        if (!pKFn->isBad() && pKFn->mnId < pKF->mnId) {
          ...
          LoopClosingVLAD::KeyFrameAndPose::const_iterator itn =
